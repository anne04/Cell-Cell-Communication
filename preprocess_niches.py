...
for op_index in range (2, len(options_list)):
    print('%d'%op_index)
    ...
    data_list_pd.to_csv('/cluster/projects/schwartzgroup/fatema/find_ccc/synthetic_cell_'+options+'_x.csv', index=False, header=False)
    data_list_pd = pd.DataFrame(temp_y)        
    data_list_pd.to_csv('/cluster/projects/schwartzgroup/fatema/find_ccc/synthetic_cell_'+options+'_y.csv', index=False, header=False)
