...
for op_index in range (0, len(options_list)):
    ...
    data_list_pd_x = pd.DataFrame(temp_x)        
    data_list_pd_x.to_csv('/cluster/projects/schwartzgroup/fatema/find_ccc/synthetic_cell_'+options+'_x.csv', index=False, header=False)
    data_list_pd_y = pd.DataFrame(temp_y)        
    data_list_pd_y.to_csv('/cluster/projects/schwartzgroup/fatema/find_ccc/synthetic_cell_'+options+'_y.csv', index=False, header=False)
    ...
    data_list_pd.to_csv('/cluster/projects/schwartzgroup/fatema/find_ccc/synthetic_gene_vs_cell_'+options+'.csv')
