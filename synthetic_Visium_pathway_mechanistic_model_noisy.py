...
options = 'equidistant_mechanistic_noise'+str(noise_percent)+'level'+str(noise_add)

with gzip.open("/cluster/projects/schwartzgroup/fatema/find_ccc/" + options +'_'+'cellvsgene', 'wb') as fp:
    pickle.dump(cell_vs_gene, fp)
    
with gzip.open("/cluster/projects/schwartzgroup/fatema/find_ccc/" + options +'_cellvsgene_'+ 'not_quantileTransformed', 'wb') as fp:
    pickle.dump(cell_vs_gene_notNormalized, fp)

with gzip.open("/cluster/projects/schwartzgroup/fatema/find_ccc/" + options + '_input_graph', 'wb') as fp:  # at least one of lig or rec has exp > respective knee point          
    pickle.dump([row_col, edge_weight, lig_rec], fp)

random_activation = []
with gzip.open("/cluster/projects/schwartzgroup/fatema/find_ccc/" +  options + '_ground_truth', 'wb') as fp:  # at least one of lig or rec has exp > respective knee point          
    pickle.dump([lr_database, lig_rec_dict_TP, random_activation], fp)

ccc_region = active_spot
with gzip.open("/cluster/projects/schwartzgroup/fatema/find_ccc/" +  options +'_coordinate', 'wb') as fp:
    pickle.dump([temp_x, temp_y, ccc_region], fp)

cell_vs_lrgene = cell_vs_gene[:,0:lr_gene_count]
with gzip.open("/cluster/projects/schwartzgroup/fatema/find_ccc/" + options +'_'+'cellvslrgene', 'wb') as fp:
...
fp = gzip.open("/cluster/projects/schwartzgroup/fatema/find_ccc/" + options + '_ground_truth', 'rb')  # at least one of lig or rec has exp > respective knee point          
...
with gzip.open("/cluster/projects/schwartzgroup/fatema/find_ccc/" + 'Tclass_synthetic_data_ccc_roc_control_model_'+ options , 'rb') as fp:  # +'_'+'notQuantileTransformed'at least one of lig or rec has exp > respective knee point          
    lr_database, lig_rec_dict_TP_temp, random_activation = pickle.load( fp)
...
for i in range (0, datapoint_size):
    for j in range (0, datapoint_size):
        #if i==j: 
        #    continue
        if i in lig_rec_dict_TP_temp and j in lig_rec_dict_TP_temp[i] and len(lig_rec_dict_TP_temp[i][j]) > 0:
            for k in range (0, len(lig_rec_dict_TP_temp[i][j])):
                lig_rec_dict_TP[i][j].append(lig_rec_dict_TP_temp[i][j][k])
                P_class = P_class + 1
